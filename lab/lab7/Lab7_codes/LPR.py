...
class ProjectController(app_manager.RyuApp):
    ...
    def _packet_in_handler(self, ev):
        ...
        def dst_agg_for_core(dst_pod, core):
            # core17/18 -> odd agg; core19/20 -> even agg
            ...
            if sp == dp:
                ...
                for agg in aggs_in_pod(sp):
                    for core in cores_for_agg(agg):
                        paths.append([src_edge, agg, core, dst_agg_for_core(dp, core), dst_edge])

        # LPR 字典序
        path = min(paths, key=lambda p: tuple(p))

        # 未收敛
        ok = True
        for i in range(len(path) - 1):
            if path[i + 1] not in self.adjacency[path[i]]:
                ok = False
                break
        if not ok:
            return

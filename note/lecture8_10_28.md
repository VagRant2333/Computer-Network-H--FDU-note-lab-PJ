...
**实际上，W-cycle图中是指数上升，W-time图则是凸函数形似根号x，而且最开始会有抖动（因为最开始加一分之一，之后二分之一...越往后细粒度越高；同时拥塞控制时因为RTT变化所以W-time图也不是完全的直线，会类似凸函数**
...
**TCP-Reno再解读**
慢启动阶段
初始状态：cwnd=1（或 MSS），ssthresh 设为较大初始值（如 65535 字节）。
变化规则：cwnd 按指数增长（每经过一个 RTT 翻倍），直到 cwnd ≥ ssthresh，进入拥塞避免阶段。
拥塞避免阶段
变化规则：cwnd 按线性增长（每经过一个 RTT 加 1 MSS），逐渐逼近网络承载上限。
触发拥塞：若检测到超时（重传计时器到期），则 ssthresh = 当前 cwnd/2，cwnd 重置为 1，重新进入慢启动。
快速重传与快速恢复阶段
触发条件：收到 3 个重复确认，判定为轻度拥塞。
变化规则：ssthresh = 当前 cwnd/2，cwnd=ssthresh（而非重置为 1），直接进入拥塞避免阶段，避免性能骤降
